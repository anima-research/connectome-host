/**
 * Peek panel — shows live events for a single agent scope (in-process subagent
 * or fleet child). The server delivers them as `peek` messages (subagents) or
 * already-streaming `child-event` messages (fleet children); we render both
 * uniformly here.
 */
...
export function PeekPanel(props: {
  scope: string;
  lines: PeekLine[];
  onClose(): void;
  onStop?: () => void;
  ...
  return (
    <div class="border-l border-neutral-800 w-96 shrink-0 bg-neutral-950 flex flex-col h-full">
      <div class="border-b border-neutral-800 px-3 py-2 flex items-center gap-2">
        <span class="text-[10px] uppercase tracking-wider text-neutral-500 font-semibold">peek</span>
        <span class="font-mono text-sm text-neutral-200 truncate">{props.scope}</span>
...
/** Convert a peek/child stream event into a renderable line. Returns null
 *  to mean "skip" (e.g. token events that the caller folds elsewhere). */
export function formatPeekEvent(event: { type: string; [k: string]: unknown }): PeekLine | null {
  const get = <T,>(k: string): T | undefined => event[k] as T | undefined;
  const id = nextPeekLineId();
  switch (event.type) {
    case 'inference:started':
    case 'tokens':                       // server -> client folds tokens; see App.tsx
    ...
    default:
      // Show unknown events as raw type for diagnostics.
      return { id, kind: 'event', text: `· ${event.type}`, color: 'text-neutral-600' };
  }
}

let peekIdCounter = 0;
function nextPeekLineId(): number { return ++peekIdCounter; }
